...
export interface TileSet {
    id: string;
    name: string;
    lightTiles: string; // If isTerrain lightTiles == Terrain RGB
    darkTiles: string;  // if isTerrain: darkTiles == Hill Shading 
    isTerrain: boolean;
}
...
class TileSetModal extends Modal {
    ...
    constructor(app: App, tileSet: TileSet | null, onSave: (tileSet: TileSet) => void, terrainDEM: boolean | false) {
        super(app);
        this.isNew = !tileSet;
        this.tileSet = tileSet || {
            ...
            lightTiles: '',
            darkTiles: '',
            isTerrain: terrainDEM,
        };
        this.isTerrain = terrainDEM;
        this.onSave = onSave;
    }
    ...
    onOpen() {
        ...
        const lightModeSetting = new Setting(contentEl)
            .setName((!this.isTerrain) ? 'Light mode' : 'Terrain Raster DEM')
            .addText(text => text
        ...
        if (this.isTerrain)
            lightModeSetting.descEl.innerHTML = 'Tile URL for Terrain Raster DEM. See the <a href="https://help.obsidian.md/bases/views/map">Map view documentation</a> for examples.';
        else
        ...
        new Setting(contentEl)
            .setName((!this.isTerrain) ? 'Dark mode (optional)' : 'Terrain Hill shading (optional)')
            .setDesc((!this.isTerrain) ?
                'Tile URL or style URL for dark mode. If not specified, light mode tiles will be used.' :
                'Tile URL for Hill shading')
            .addText(text => text
...
export class MapSettingTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        const backgroundListContainer = containerEl.createDiv('map-tileset-list');

        new Setting(containerEl)
            .setHeading()
            .setName('Terrain')
            .addButton(button => button
                .setButtonText('Add Terrain')
                .setCta()
                .onClick(() => {
                    new TileSetModal(this.app, null, async (tileSet) => {
                        this.plugin.settings.tileSets.push(tileSet);
        ...
        const terrainListContainer = containerEl.createDiv('terrain-tileset-list');

        this.plugin.settings.tileSets.forEach((tileSet, index) => {
            if (tileSet.isTerrain)
                this.displayTileSetItem(terrainListContainer, tileSet, index);
            else
                this.displayTileSetItem(backgroundListContainer, tileSet, index);
        });

        if (this.plugin.settings.tileSets.length === 0) {
            backgroundListContainer.createDiv({
